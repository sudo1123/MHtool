...
while True:
    ...
    if j1>0:
        ...
        a = input("请输入需要选择的功能的代码(0是生成质数表，1是翻倍运算，2是计算算数平方根，3是计算圆周率，4是求n次方，5是四则运算，6是生成斐波那契数列，7是计算黄金分割率,8是找零程序（贪心算法），9是统计，10是绘制函数图像，11是考试成绩数据整理，12是绘制正弦/余弦函数图像，输入clock启动计时器，输入exit关闭程序，输入help查看使用指南,输入rr查看历史计算结果，输入cr清空历史计算结果，输入history查看操作历史，输入ch清空操作历史）")
        j1+=1
    #生成质数表
    if a=="0":
...
#绘制正弦/余弦函数图像
    if a=="12":
        ...
        acc3=int(input("输入缩放系数（推荐1000）"))
        acc4=int(input("设置图像起点（推荐-1000）"))
        acc5=10*acc3/((360/acc1*acc2)/(2*pi))
        ...
        for r in range(int(360/acc1)):
            step1=r*acc5
            step2=acc5*float(Y_co[r])
            turtle.penup()
            turtle.goto(step1,step2)
            turtle.pendown()
            turtle.forward(1)
        history("12")
#帮助
    if a=="help":
        print("请输入需要选择的功能的代码(0是生成质数表，1是翻倍运算，2是计算算数平方根，3是计算圆周率，4是求n次方，5是四则运算，6是生成斐波那契数列，7是计算黄金分割率,8是找零程序（贪心算法），9是统计，10是绘制函数图像，11是考试成绩数据整理，12是绘制正弦/余弦函数图像，输入clock启动计时器，输入exit关闭程序，输入help查看使用指南，输入rr查看历史计算结果，输入cr清空历史计算结果，输入history查看操作历史，输入ch清空操作历史)")
#读取计算结果
    if a=="rr":
